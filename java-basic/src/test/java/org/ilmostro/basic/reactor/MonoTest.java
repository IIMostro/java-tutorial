...
public class MonoTest {
  ...
  public void delay() throws InterruptedException {
    Mono.just("hello").delaySubscription(Duration.ofSeconds(1)).subscribe(log::info);
  ...
  public void test1() throws InterruptedException {
    ...
    Schedulers.fromExecutor(scheduledExecutorService)
        .schedulePeriodically(
            () -> log.info("current:{}", System.currentTimeMillis()), 1, 1, TimeUnit.SECONDS);
  ...
  public void subscriber() throws Exception {
    Flux.just(new int[] {1, 2, 3, 4})
        .metrics()
        .flatMapIterable(
            (Function<int[], Iterable<?>>)
                ints -> Arrays.stream(ints).boxed().collect(Collectors.toList()))
  ...
  public void checkpoint() {
    Mono.just(2).map(v1 -> v1 + 1).checkpoint("description", true).log(logger).subscribe();
  }
