package org.ilmostro.basic.datastruct.stream;
...
public class StreamTest {
  ...
  private final List<User> users =
      Stream.generate(User::supplier).skip(10).limit(20).collect(Collectors.toList());
  ...
  public void generator() {
    ...
    Map<Integer, List<String>> nameMapByAge =
        collect.stream()
            .collect(
                Collectors.groupingBy(
                    User::getAge, Collectors.mapping(User::getName, Collectors.toList())));
  ...
  public void collect() {
    Map<String, User> collect =
        users.stream().collect(Collectors.toMap(User::getPassword, v1 -> v1, (v1, v2) -> v1));
  }
  ...
  public void parallel() {
    ...
    // users.stream().parallel().forEach(var1 -> log.info(Thread.currentThread().getName())));
    //        submit.join();
  ...
  @Test
  public void filter() {
    users.stream().filter(var1 -> var1.getAge() > 18).map(User::toString).forEach(log::info);
  }
  ...
  public void count() {
    Map<String, BigDecimal> collect =
        users.stream()
            ...
            .collect(
                Collectors.toMap(User.Score::getSubject, User.Score::getScore, BigDecimal::add));
    for (Map.Entry<String, BigDecimal> entry : collect.entrySet()) {
      log.info("key:{}", entry.getKey());
      log.info("value:{}", entry.getValue());
    }
