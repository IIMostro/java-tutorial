...
public class HashTests {
  ...
  public void basic() {
    ...
    final List<User> users =
        Stream.generate(User::supplier).limit(count).collect(Collectors.toList());
    watch.stop();
    log.info("create user:[{}]", watch.getTime(TimeUnit.MILLISECONDS));
    watch.reset();
    watch.start();
    final List<Integer> collect =
        users.stream().map(Objects::hashCode).collect(Collectors.toList());
    watch.stop();
    log.info("basic hash user:[{}]", watch.getTime(TimeUnit.MILLISECONDS));
    watch.reset();
    watch.start();
    final List<Integer> hash =
        users.stream()
            .map(User::toString)
            .map(v1 -> Hashing.murmur3_128().hashString(v1, StandardCharsets.UTF_8))
            .map(HashCode::asInt)
  ...
  public void murmur() {
    ...
    final HashCode hashCode =
        Hashing.murmur3_128().hashString(user.toString(), StandardCharsets.UTF_8);
