...
public class BitTest {
  ...
  public void test1() {
    ...
    logger.info(
        "status >> 2 [{}] & 1<<2[{}], value:[{}]",
        Integer.toBinaryString(status >> 2),
  ...
  public void test2() {
    ...
    for (int i = 0; i < 9; i++) {
      logger.info(
          ...
          i,
          Integer.toBinaryString(number >> i),
          number >> i);
    }
  ...
  public void test4() {
    ...
    for (int i = 0; i < 5; i++) {
      logger.info(
          "1 << {} & {} = {}",
          i,
          Integer.toBinaryString(parameter),
          Integer.toBinaryString((1 << i) & parameter));
    }
  }
  ...
  void test_unsigned_2_signed() {
    final var buffer = PooledByteBufAllocator.DEFAULT.buffer();
    buffer.writeShort(-103);
    logger.info("{}", ByteBufUtil.prettyHexDump(buffer));
    logger.info("{}", buffer.readUnsignedShortLE());
  }
}
