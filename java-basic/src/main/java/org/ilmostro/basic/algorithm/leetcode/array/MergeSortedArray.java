...
public class MergeSortedArray {
    ...
    public static void main(String[] args) {
        int[] arr1 = {2, 0};
        int[] arr2 = {1};
        new MergeSortedArray().optimize(arr1, 1, arr2, 1);
        for (int i : arr1) {
    ...
    public void merge(int[] nums1, int m, int[] nums2, int n) {
        ...
        for (int i = 0; i < merge.length; i++) {
            if(v1 >= m) merge[i] = nums2[v2++];
            else if (v2 >= n) merge[i] = nums1[v1++];
            else if (nums1[v1] > nums2[v2]) merge[i] = nums2[v2++];
            else merge[i] = nums1[v1++];
        }
